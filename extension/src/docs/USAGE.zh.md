...
- 映射模板默认保存在本地浏览器（不上传服务端）：包含表头、统一字段与映射关系，可用于下次自动套用
...
当你处理的是高频场景（例如两表 Join、对账、统一导出），建议把映射保存为模板，下一次遇到同结构文件可以直接套用，减少重复分析与人工调整。

### 6.1 保存模板

- 先完成一次“智能分析与映射”（或手动调整映射）
- 在右侧映射区右上角点击“保存模板”，为模板命名

### 6.2 自动命中与套用

- 当你再次上传表头结构相同的文件时，系统会提示“发现模板”
- 你可以点击“应用模板”直接生成映射结果，也可以点“重新分析”走 AI

### 6.3 模板存储位置与清除

- 默认存储在本地浏览器（Local Storage），同一设备/浏览器可复用
- 模板可能会在以下情况消失：清理浏览器缓存/站点数据、使用无痕模式、重装浏览器/更换设备
- 清除方式：浏览器站点设置中清除本应用的本地数据，或清除对应站点的存储
- 存储内容仅包含：模板名称、表头、统一字段、映射关系与可能的 AI 转换逻辑；不包含任何单元格数据
- 数量上限：默认最多保留 50 个模板，超出后会按“最近使用”自动清理最旧的模板
...
- 鉴权方式（满足任意一种即可）：
  - **固定管理员 Token**：配置 `ADMIN_TOKEN` 后，访问 `/admin?token=<ADMIN_TOKEN>`（或请求头带 `x-admin-token`）
  - **管理员账号**：配置 `ADMIN_DEFAULT_EMAIL` + `ADMIN_DEFAULT_PASSWORD`（服务启动会自动创建 plan=admin 的账号）
  - **管理员邮箱白名单**：配置 `ADMIN_EMAILS`（逗号或空格分隔），白名单邮箱登录后会自动升级为 `plan=admin`，并显示“后台管理”入口
- 支持按天 / 按周 / 按月查看：
  - 活跃用户（去重 clientId）
...
- 提交后会写入服务端数据库（feedback 表），用于产品迭代分析
